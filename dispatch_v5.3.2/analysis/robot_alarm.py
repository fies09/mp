...
class FourFoldAlarm(object):
    ...
    def get_alarm_type(self, robot_algorithm_param_id, robot_position_id, item_name):
        ...
        logger.info(start_u)
        logger.info(type(start_u))
        self.start_u = start_u

        core_device_part_data = DBCoreDevicePart().get_core_device_data(robot_position_id, start_u)
        logger.info(core_device_part_data)
        ...
        self.core_device_part_id = core_device_part_data.get("core_device_part_id")
        ...
        if alarm_light_data:
            return item_id, int(alarm_light_data.get("value")), alarm_light_data.get("conditions"), \
                   alarm_light_data.get("level"), alarm_light_data.get("rule_id"), str(start_u)
        return False, None, None, None, None, None

    # 四倍对比规则(入口)
    def contrast_alarm(self, task_data, env_name, env_value, robot_algorithm_param_id):
        task_data["alarm_type"] = 3
        robot_position_id = task_data.get("robot_position_id")
        ...
        logger.info(env_value)
        logger.info(alarm_data_conditions)
        logger.info(alarm_data_rule_start_u)

        task_data["value"] = env_value
        task_data["alarm_u"] = str(alarm_data_rule_start_u)
        if not item_id:
            return False

        alive_data = self.db_alarm_manage.get_alarm_light_data(robot_position_id, item_id, str(alarm_data_rule_start_u))
        logger.info("alive_data")
        logger.info(alive_data)
    ...
    def update_alarm_num(self, alive_data, img_path):
        ...
        DBAlarmManage().update({"alarm_manage_id": alarm_manage_id}, alive_data)

    # 更新正常次数
    def update_alarm_common_num(self, alive_data, rule_id):
        logger.info("更新四倍相机正常次数")
        logger.info(alive_data)
        alarm_rule_data = self.db_alarm_rule.get_by_rule_id(rule_id)
        alarm_manage_id = alive_data.get("alarm_manage_id")
        common_num = alive_data.get("common_num")
    ...
    def insert_alarm(self, task_data, num, robot_item_id, level, alarm_desc):
        ...
        ret["alarm_type"] = task_data.get("alarm_type")
        ret["num"] = num
        ret["core_device_part_id"] = self.core_device_part_id
        ret["alarm_u"] = self.start_u
        ret["assign"] = 0
        ret["img_path"] = task_data.get("img_path")
...
class IndustryAlarm(object):
    ...
    def contrast_alarm(self, task_data, env_name, env_value):
        ...
        task_data["value"] = env_value
        logger.info(alarm_data_level)
        logger.info(alarm_data_undo_alarm)
        logger.info(alarm_data_rule_start_u)

        if not item_id:
            logger.error("工业相机告警对比错误，无item_id数据")
            return False
        alive_data = DBAlarmManage().get_alarm_light_data(robot_position_id, item_id, str(alarm_data_rule_start_u))
        logger.info("alive_data")
        logger.info(alive_data)
        if alive_data:
            alive_data["inspect_project_detail_id"] = task_data.get("inspect_project_detail_id")
    ...
    def update_alarm_num(self, alive_data, img_path):
        ...
        logger.info("更新告警数据{}".format(alive_data))
    ...
    def update_alarm_common_num(self, alive_data, alarm_data_undo_alarm):
        ...
        common_num = alive_data.get("common_num")
        common_num += 1
        logger.info("更新正常次数数据{}".format(alive_data))
...
class PowerAlarmAL(object):
    ...
    def contrast_alarm(self, task_data, env_name, env_value):
        ...
        if not data_state:
            ...
            return False
        db_alarm_manage = DBAlarmManage()
        for alarm_data_data in alarm_data_list:
            logger.info(robot_position_id)
            logger.info(power_robot_item_id)
            logger.info(power_device_id)
            alive_data = db_alarm_manage.get_power_alarm_data(robot_position_id, power_robot_item_id,
                                                              power_device_id)
            logger.info(alive_data)
...
class PointerAlarmAL(object):
    ...
    def contrast_alarm(self, task_data, env_name, env_value):
        ...
        logger.info(env_value)
        if "开关" in env_name:
            task_data["alarm_type"] = 12
        else:
            task_data["alarm_type"] = 11
        self.alarm_desc = env_name + ":" + str(env_value)
