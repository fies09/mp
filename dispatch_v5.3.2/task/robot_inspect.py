...
handler = logging.FileHandler("/var/log/dispatch_logs/robot_inspect.log")
...
server_data = {
    "ip": "10.173.26.159",
    "all_port": [[8086, "agx"], [8088, "ips"], [8087, "arithmetic"]]
}
        ...
        for port in ["2007", "2009"]:
            ...
            if xml_data.status_code != 200:
                ...
                if port == "2007":
                    ...
                    robot_voice("上四倍相机检测异常")
                else:
                    logger.error("下四倍相机检测异常")
                    robot_voice("下四倍相机检测异常")
            else:
                if port == "2007":
...
def server_test():
    ...
    for i_port in all_port:
        ...
        if not res:
            ...
            robot_voice("{}服务检测异常".format("算法" if i_port[1] == "arithmetic" else i_port[1]))
            pd_list.append(False)
        else:
...
def inspect_main(type):
    if type == 1:
        ...
        if res:
            ...
            logger.info("======================传感器检测异常======================")
        return_data = inspect_res["sensor"]
        return return_data
    elif type == 2:
        logger.info("======================执行摄像头检测======================")
        res = camera_test()
        ...
        return_data = inspect_res["camera"]
        return return_data
    elif type == 3:
        logger.info("======================执行状态检测======================")
        res = state_test()
